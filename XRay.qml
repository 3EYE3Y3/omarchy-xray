...
Item {
  ...
  function loadReport(raw) {
    try {
      ...
      for (var key in sections) {
        if (["capabilities", "actions"].indexOf(key) === -1)
          names.push(key);
      }
      sectionNames = names;
  ...
  function addRows(value, prefix, depth) {
    ...
    if (simple(value)) {
      ...
      for (var i = 0; i < Math.min(value.length, 300); i++)
        addRows(value[i], "[" + (i + 1) + "]", depth + 1);
      if (value.length > 300)
        detailModel.append({
                             label: "limit",
                             value: (value.length - 300) + " more rows",
                             depth: depth
                           });
    } else if (value) {
      for (var key in value)
        addRows(value[key], String(key).replace(/_/g, " "), depth + 1);
    }
  }
  ...
  function refreshVision() {
    if ((!vision && !hud) || visionProcess.running)
      return;
  ...
  IpcHandler {
    ...
    function status(): string {
      return JSON.stringify({
                              ...
                              hud: root.hud,
                              hudPid: root.hudPid,
                              windows: root.visionRows.length
                            });
    }
  ...
  Process {
    ...
    onExited: function (exitCode) {
      root.loading = false;
      if (dataOut.text)
        root.loadReport(dataOut.text);
      else
        root.errorText = String(dataErr.text || "Probe failed").trim().substring(0, 300);
    }
  }
  ...
  Timer {
    ...
    onTriggered: if (!dataProcess.running)
                   root.inspect(root.activeTarget)
  }
      ...
      Item {
        ...
        Keys.onPressed: function (event) {
          if (event.key === Qt.Key_Escape) {
            ...
            root.inspect(root.activeTarget);
            event.accepted = true;
          } else if (event.key === Qt.Key_1 && root.sectionNames.length > 0) {
            root.selectSection(root.sectionNames[0]);
            event.accepted = true;
          }
        }
        ...
        ColumnLayout {
          ...
          RowLayout {
            ...
            ColumnLayout {
              ...
              Text {
                text: root.typeText + "  ·  LOCAL / READ ONLY"
                color: root.accent
                font.family: root.fontFamily
            ...
            Text {
              text: root.loading ? "SCANNING" : "LIVE"
          ...
          RowLayout {
            ...
            Rectangle {
              ...
              Text {
                ...
                text: "PID, :port, path, domain, IP, interface…"
                color: root.muted
                font.family: root.fontFamily
                font.pixelSize: Style.font.body
              }
              ...
              Repeater {
                ...
                delegate: Rectangle {
                  ...
                  Text {
                    ...
                    color: root.selectedSection === modelData ? root.accent : root.muted
                    font.family: root.fontFamily
                    font.pixelSize: Style.font.caption
                    font.bold: true
                  }
            ...
            ListView {
              ...
              delegate: Rectangle {
                ...
                RowLayout {
                  ...
                  spacing: Style.spacing.md
                  Text {
                    Layout.preferredWidth: Math.min(Style.space(260), parent.width * 0.35)
          ...
          Text {
            Layout.fillWidth: true
            text: "CTRL+R REFRESH  ·  NO TELEMETRY  ·  NO SUDO"
      ...
      Repeater {
        ...
        delegate: Rectangle {
          ...
          Column {
            ...
            Text {
              width: parent.width
              text: String(modelData.application || "window").toUpperCase()
              color: root.accent
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              font.bold: true
            ...
            Text {
              width: parent.width
              text: "PID " + modelData.pid + "   CPU " + modelData.cpu_percent + "%   RAM " + modelData.memory
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              elide: Text.ElideRight
            ...
            Text {
              width: parent.width
              text: "CONNECTIONS " + modelData.connections + "   THREADS " + modelData.threads
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              elide: Text.ElideRight
