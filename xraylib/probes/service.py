...
class ServiceProbe(Probe):
    ...
    def collect(self, target: Target) -> dict[str, Any]:
        unit = (
            target.identifier
            if target.identifier.endswith(".service")
            else f"{target.identifier}.service"
        )
        ...
        logs = self.runner.run(
            ["journalctl", "--no-pager", "--quiet", "--unit", unit, "--lines", "30"],
            timeout=4,
            max_output=128_000,
        )
        return {
            ...
            "dependencies": {
                name: properties.get(name, "").split() for name in ("Requires", "Wants", "After")
            },
            ...
            "actions": [
                {
                    ...
                    "available": False,
                    "reason": "Read-only acceptance candidate",
                },
                {
                    "id": "stop",
                    "confirmation_required": True,
