...
class DomainProbe(Probe):
    ...
    def collect(self, target: Target) -> dict[str, Any]:
        ...
        route = self.runner.run(["ip", "route", "get", addresses[0]], timeout=2) if addresses else None
        traceroute = self.runner.run(["tracepath", "-n", "-m", "5", host], timeout=5) if self.runner.available("tracepath") else None
        return {
            "overview": {"domain": host, "resolved": bool(addresses), "https_url": f"https://{host}"},
            "dns": {
            ...
            "raw": {"network_requested": True, "timeouts_seconds": {"dns": 2, "tls": 3, "trace": 5}},
        }
...
class IpProbe(Probe):
    ...
    def collect(self, target: Target) -> dict[str, Any]:
        ...
        ping = self.runner.run(["ping", ping_flag, "-n", "-c", "1", "-W", "2", str(address)], timeout=3)
        ...
        return {
            "overview": {
                "address": str(address), "version": address.version, "private": address.is_private,
                "loopback": address.is_loopback, "link_local": address.is_link_local,
                "multicast": address.is_multicast, "global": address.is_global,
            },
            "reachability": {"reachable": ping.ok, "summary": ping.stdout.splitlines()[-1] if ping.stdout else ping.stderr.strip()},
            ...
            "raw": {"network_requested": True, "timeouts_seconds": {"ping": 3, "dns": 2, "route": 2}},
        }
