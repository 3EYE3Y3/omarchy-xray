...
class SystemProbe(Probe):
    ...
    def collect(self, target: Target) -> dict[str, Any]:
        ...
        processes = self.runner.run(
            ["ps", "-eo", "pid=,comm=,%cpu=,%mem=,rss=", "--sort=-%cpu"], timeout=3
        )
        ...
        return {
            ...
            "raw": {
                "meminfo": memory,
                "sources": ["/proc", "df", "ps", "lspci", "/sys/class/thermal", "ip"],
            },
        }
    ...
    def _temperatures() -> list[dict[str, Any]]:
        rows = []
        for path in (
            sorted(os.listdir("/sys/class/thermal")) if os.path.isdir("/sys/class/thermal") else []
        ):
            ...
            if raw.lstrip("-").isdigit():
                rows.append(
                    {
                        "sensor": read_text(f"{base}/type", limit=128).strip() or path,
