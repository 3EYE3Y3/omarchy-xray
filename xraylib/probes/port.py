...
def exposure(address: str) -> str:
    if address in {"127.0.0.1", "::1", "localhost"}:
        return "local only"
    if address in {"0.0.0.0", "*"}:  # noqa: S104 - classifies an observed bind address
...
class PortProbe(Probe):
    ...
    def collect(self, target: Target) -> dict[str, Any]:
        ...
        connections = [
            row
            for row in parse_ss(completed.stdout)
            if row["local_port"] == port or row["remote_port"] == port
        ]
        for row in connections:
            row["exposure"] = exposure(row["local_address"])
            row["url"] = (
                f"http://{row['local_address']}:{port}"
                if row["state"] in {"LISTEN", "UNCONN"}
                else ""
            )
        return {
            "overview": {
                "port": int(port),
                "found": bool(connections),
                "listeners": sum(row["state"] in {"LISTEN", "UNCONN"} for row in connections),
            },
            "connections": connections,
            "raw": {
                "source": completed.argv,
                "error": completed.stderr if not completed.ok else "",
            },
        }
